...
>[!info] 
>
> `$0`是 Shell 脚本的名称。所以实际脚本的参数是从 `$1` 开始的。

使用 `$` 符号获取参数，默认情况最大为 9 个参数，即`$1`~`$9`。要取第 10、第 11 个等，有两种方式处理：

1. 使用 `{}` 方式，如`${10}`、`${11}`
2. 配合使用 [shift](#shift) 命令
...
`shift` 命令，是将第 1 个参数删除，并将所有剩余参数左移一位。

这个命令一般作用：

1. 接受超过 9 个参数的时候，要腾出位置接受第 10 参数
2. 因为需求，需要取部分参数时
