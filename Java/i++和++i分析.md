...
第 **12** 行 `iload_2` 指令： 是将局部变量表的数据加载进操作数栈，这时局部变量表 j 的数据是 1，加载进栈顶，这时操作数栈 j 的数据是 1

第 **13** 行指令 `istore_2` 指令： 将操作数栈的数据保存进局部变量表，这时局部变量表 j 的数据仍为 1
