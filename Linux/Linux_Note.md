...
### <span id="linux_dirfile_rm">删除</span>
...
`rm -r` 这个命令删除目录时，会会删除指定目录及其所有文件，当然在删除前什么询问是否确定删除。

如果不想逐个确认删除，可以加个 `-f` 选项，意思为强制删除。
> [!info] 
> 
> 强制删除目录完全命令是：`rm -rf 目录`。

#### rmdir

`rmdir` 命令可以用来删除目录。

如果目录中包含任何文件和子目录，`rmdir` 不会继续进行处理，这可以避免误删文件的可能。
...
`ln` 命令功能是为某一个文件在另外一个位置建立一个同上涌的链接。

需要在不同目录，用到相同的文件时，不需要在每一个需要的目录下，都放一个相同的文件，只需要在某个固定的目录，放上该文件，在其他目录下，用 `ln` 命令链接它就可以了，这种方式不必重复占用磁盘空间。

创建一个链接使用 `ln` 命令。语法：`ln from to`，即`ln 源文件或目录 目标文件或目录`

链接分为两种：

* [硬链接](#硬链接)：一个档案可以有多个名称
* [软链接](#软链接)：产生一个特殊的档案，该档案的内容是指向另一个档案的位置。

#### 软链接

软链接也称为「符号链接」，命令就加个 `s` 参数，即`ln -s`。

软链接特点：

* 软链接，以路径形式存在，类似 Windows 中的快捷方式
* 软链接可以跨文件系统
* **软链接可以对一个不存在的文件名进行链接**
* **软链接可以对目录进行链接**
