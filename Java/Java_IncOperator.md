...
![image-20191102214024405](Java_IncOperator.assets/image-20191102214024405.png)
...
![image-20191102213934082](Java_IncOperator.assets/image-20191102213934082.png)
...
对比无赋值的 [例子 1](#例子%201)，本例子指令中多了 `istore` 指令，`store` 指令是将操作数栈数据出栈保存到**在前，那就先对局部变量表进行自增，**中，也就是在函数中说「赋值」实际是「赋」到「局部变量表」。
...
![image-20191102213443042](Java_IncOperator.assets/image-20191102213443042.png)
...
++j，第**局部变量表**、**47**行指令可以看出先对局部变量表中 j 的数据自增为 1，然后才将局部变量表的数据 load 进操作数栈，最后 println() 方法调操作数栈数据，所以 j 的结果显示为 1。
...
![image-20191103082103712](Java_IncOperator.assets/image-20191103082103712.png)
...
而 ++j 对应的是第**50**~第**8**行指令，与上面的 i++ 完全一样。
...
![image-20191103084315564](Java_IncOperator.assets/image-20191103084315564.png)
...
![image-20191104030012270](Java_IncOperator.assets/image-20191104030012270.png)
